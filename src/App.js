                ...
                <div className="max-w-7xl mx-auto">
                    <header className="flex justify-between items-center mb-6 pb-4 border-b border-gray-700">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={() => setIsFoodModalOpen(true)} className="ml-4 bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-3 rounded-lg"><EditIcon className="w-6 h-6"/></button>
                        </div>
                    </header>
...
const KitchenCard = ({ group, foodItems, onUpdateFood }) => {
    ...
    const dietarySummary = Object.entries(dietary || {}).filter(([, count]) => count > 0).map(([key, count]) => `${DIETARY_OPTIONS[key]}: ${count}`).join(', ');

    return (
        <div className="bg-gray-900 rounded-2xl shadow-lg border border-gray-800 flex flex-col">
            <div className="p-4 border-b border-gray-800"><div className="flex justify-between items-center"><h2 className="text-2xl font-bold text-white flex-grow">{teamName}</h2><div className="flex items-center gap-2"><span className="bg-gray-800 px-3 py-1 rounded-md text-2xl font-bold">{teamSize}</span><span className="font-mono bg-gray-800 px-3 py-1 rounded-md text-lg">{formatTime(time)}</span></div></div>{dietarySummary && <p className="text-base text-amber-400 mt-2 text-center">{dietarySummary}</p>}</div>
        ...
        <div>
            ...
            <div className="space-y-2">
                {Object.entries(items).map(([key, name]) => {
                    ...
                    return (<div key={key} className="flex justify-between items-center bg-gray-800 p-2 rounded-lg"><span className="text-white">{name}</span><div className="flex items-center gap-3"><button onClick={() => onUpdateFood(key, count, 'remove')} className="w-8 h-8 flex items-center justify-center bg-red-600 hover:bg-red-700 rounded-md text-xl font-bold">-</button><span className="text-2xl font-bold text-white w-8 text-center">{count}</span><button onClick={() => onUpdateFood(key, count, 'add')} className="w-8 h-8 flex items-center justify-center bg-green-600 hover:bg-green-700 rounded-md text-xl font-bold">+</button></div></div>);
                })}
            </div>
        </div>
    );
